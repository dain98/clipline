...
- Remote integration continuity: with real accounts and a League match, load the Cloud library/profile/avatar, run the osu! connection test and recent-play enrichment, and record League markers. Briefly interrupt networking or the local League endpoint during each flow; confirm commands fail within their documented timeout instead of hanging, retry succeeds after recovery, large uploads/downloads continue while making progress, and the Cloud library reports truncation if the server exposes more than 10,000 unique clips.
