...
**Execution note (2026-06-27 external validation harness):** Added
`scripts\validate-fallback-client.ps1` so the final Nate/VM gate produces evidence instead of
free-form notes. The harness launches Clipline with a fixed fallback port, optionally applies
`--debug-webview2-preflight missing` for local deterministic validation, discovers the tokenized
fallback URL, proves setup logged `webviews=[]` before fallback, verifies the shared fallback UI is
served, runs fallback invokes for settings, library, storage, game plugins, and memory, then writes
an evidence JSON. Local command validated:
