...
- **Elevated games require explicit consent.** Clipline normally remains `asInvoker`. If Windows reports that a detected game is elevated above Clipline, the warning may restart only Clipline's current executable through UAC for this launch; cancellation leaves the normal process alive, and the elevated child verifies the exact parent PID plus creation time before waiting for it to exit. The UI states that elevation resets the current replay buffer and never turns elevation into a saved startup preference.
