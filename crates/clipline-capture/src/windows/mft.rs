...
use windows::Win32::Media::MediaFoundation::{
    ...
    MFMediaType_Video, MFT_ENUM_FLAG_HARDWARE, MFT_ENUM_FLAG_SORTANDFILTER,
    ...
    MFT_MESSAGE_SET_D3D_MANAGER, MFT_OUTPUT_DATA_BUFFER, MFVideoFormat_H264, MFVideoFormat_NV12,
    MFVideoInterlace_Progressive, METransformDrainComplete, METransformHaveOutput,
    METransformNeedInput, MFSampleExtension_CleanPoint, MF_EVENT_FLAG_NO_WAIT,
...
use crate::windows::nv12::VideoConverter;
...
impl MftH264Encoder {
    ...
    pub fn new(
        ...
        unsafe {
            out_ty.SetGUID(&MF_MT_MAJOR_TYPE, &MFMediaType_Video).map_err(backend)?;
            out_ty.SetGUID(&MF_MT_SUBTYPE, &MFVideoFormat_H264).map_err(backend)?;
            out_ty.SetUINT32(&MF_MT_AVG_BITRATE, cfg.bitrate_bps).map_err(backend)?;
            out_ty
                .SetUINT64(&MF_MT_FRAME_SIZE, ((cfg.width as u64) << 32) | cfg.height as u64)
                .map_err(backend)?;
            out_ty.SetUINT64(&MF_MT_FRAME_RATE, ((cfg.fps as u64) << 32) | 1).map_err(backend)?;
            out_ty
                .SetUINT32(&MF_MT_INTERLACE_MODE, MFVideoInterlace_Progressive.0 as u32)
                .map_err(backend)?;
            out_ty.SetUINT32(&MF_MT_MPEG2_PROFILE, H264_PROFILE_HIGH).map_err(backend)?;
            transform.SetOutputType(output_id, &out_ty, 0).map_err(backend)?;
        }
        ...
        unsafe {
            transform.ProcessMessage(MFT_MESSAGE_NOTIFY_BEGIN_STREAMING, 0).map_err(backend)?;
            transform.ProcessMessage(MFT_MESSAGE_NOTIFY_START_OF_STREAM, 0).map_err(backend)?;
        }

        let converter = VideoConverter::new(device, in_w, in_h, cfg.width, cfg.height)
    ...
    fn drain_one(&mut self) -> Result<EncodedPacket, EncodeError> {
        loop {
            ...
            let res = unsafe {
                self.transform.ProcessOutput(0, std::slice::from_mut(&mut out), &mut status)
            };
            match res {
                Ok(()) => {
                ...
                Err(e) if e.code() == MF_E_TRANSFORM_STREAM_CHANGE => {
                    ...
                    unsafe {
                        let ty = self
                            ...
                            .map_err(backend)?;
                        self.transform.SetOutputType(self.output_id, &ty, 0).map_err(backend)?;
    ...
    fn packet_from_sample(&mut self, sample: &IMFSample) -> Result<EncodedPacket, EncodeError> {
        ...
        let annexb = unsafe {
            ...
            let mut len = 0u32;
            buffer.Lock(&mut ptr, None, Some(&mut len)).map_err(backend)?;
        ...
        let (pts_s, duration_s, is_keyframe) = unsafe {
            (
                ...
                sample.GetSampleDuration().map(|d| d as f64 / 1e7).unwrap_or(nominal),
                sample.GetUINT32(&MFSampleExtension_CleanPoint).unwrap_or(0) == 1,
            )
        };
        Ok(EncodedPacket { data: annexb_to_avcc(&annexb), pts_s, duration_s, is_keyframe })
    }
...
impl Encoder for MftH264Encoder {
    fn encode(&mut self, frame: &Frame) -> Result<Vec<EncodedPacket>, EncodeError> {
        ...
        let sample = unsafe {
            ...
            sample.SetSampleTime((frame.pts_s * 1e7).round() as i64).map_err(backend)?;
            sample.SetSampleDuration((duration_s * 1e7).round() as i64).map_err(backend)?;
            sample
        };
    ...
    fn finish(&mut self) -> Result<Vec<EncodedPacket>, EncodeError> {
        ...
        unsafe {
            self.transform
                ...
                .map_err(backend)?;
            self.transform.ProcessMessage(MFT_MESSAGE_COMMAND_DRAIN, 0).map_err(backend)?;
        }
        let mut packets = Vec::new();
        loop {
...
fn variant_u32(value: u32) -> windows::Win32::System::Variant::VARIANT {
    use windows::Win32::System::Variant::{
        VARIANT, VARIANT_0, VARIANT_0_0, VARIANT_0_0_0, VT_UI4,
    };
    VARIANT {
        Anonymous: VARIANT_0 {
...
fn sequence_header_sps_pps(
    ...
    unsafe {
        ...
        media_type.GetBlob(&MF_MT_MPEG_SEQUENCE_HEADER, &mut blob, None).ok()?;
...
mod tests {
    ...
    fn encodes_synthetic_frames_to_keyframed_avcc() {
        ...
        assert!(!track.sps.is_empty() && !track.pps.is_empty(), "SPS/PPS extracted");
