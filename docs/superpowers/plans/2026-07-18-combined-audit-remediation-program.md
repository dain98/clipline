...
- Large cloud upload: upload a large original clip and a selected-audio variant using a real account. Interrupt and retry a resumable upload; confirm memory remains bounded, the remote file plays, progress resumes correctly, and local media is deleted only when the configured policy and ready-media verification both permit it. If the test deployment can throttle direct object PUTs, return 429 or 503 with a short `Retry-After`; confirm attempts are spaced (not burst), honor at least that delay, remain cancelable during the wait, and succeed or report failure within the 30-second per-wait cap.
