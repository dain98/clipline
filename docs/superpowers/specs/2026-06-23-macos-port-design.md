...
- `apps/clipline-app/src/service.rs`, `app.rs`, `games.rs`, `hotkeys.rs`, `memory.rs`, `library.rs`, `cloud.rs`, and settings persistence use Windows APIs or Windows terms directly.
...
- Baseline command on macOS: `cargo test --workspace` passed with all current tests, while `clipline-app` contributed only stub/non-platform tests.
...
| Save replay hotkey | Use `tauri-plugin-global-shortcut` on macOS for the main shortcut path. If secure-input or OS policy blocks it, report hotkey unavailable and keep tray/menu/manual save available. |
...
| Hardware/software encoding | Keep the `Encoder` trait and FFmpeg subprocess path. Add macOS encoder capabilities in the probe list, starting with FFmpeg hardware/software encoders available on the machine. Evaluate VideoToolbox as a later native backend behind the same trait. |
...
| Per-process output audio tracks | Treat as a capability. If macOS cannot provide process-isolated output tracks with stable public APIs, fallback to mixed system output plus mic and mark app-audio track splitting unsupported on that machine. |
...
| File reveal/copy | Replace Windows Explorer/CF_HDROP code with macOS Finder reveal and pasteboard file URL behavior. Prefer a small platform interface over UI branching. |
...
| Permissions | Add macOS permission checks/status for Screen Recording, Microphone, and any needed accessibility/input-monitoring path. The UI should guide the user without blocking unrelated features. |
...
5. Real macOS validation records short clips for display capture, window capture, system audio, mic audio, replay save, full-session finalize, trim export, and library playback.
6. Release validation builds a macOS `.app`/DMG or configured Tauri bundle, confirms updater metadata, and verifies startup behavior in an installed app context.

## Implementation Decomposition

This port is too large for one implementation plan. It should be split into independently testable plans:

1. **macOS app shell and platform boundaries**: run the real Tauri UI on macOS, introduce platform facade types, keep Windows behavior intact, and show capability stubs.
2. **macOS filesystem, settings, credentials, and app lifecycle**: Finder reveal, pasteboard copy, Movies default folder, Keychain cloud credential storage, startup/login status, memory status, updater/bundle config, and platform copy cleanup.
...
5. **macOS audio v1**: system audio when available, microphone capture/test, mixed output/mic track parity, and sidecar labels.
6. **macOS review/cloud parity hardening**: multitrack preview/upload behavior, file locks, playback compatibility, updater artifacts, and release verification.
...
- Per-process output audio may not have a stable public macOS equivalent to the Windows process-loopback path. The design allows a truthful fallback while preserving future room.
- Tauri/macOS WebKit playback support differs from WebView2. Automatic encoder selection must continue to prefer player-decodable codecs and warn for limited playback.
...
- Release behavior on macOS depends on signing/notarization choices. Development builds can run unsigned, but user-distributed builds need a signed/notarized path.
...
- The first implementation milestone is narrow enough to complete and verify without needing real capture.
