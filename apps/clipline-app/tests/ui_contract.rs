...
fn review_player_owns_all_controls() {
    ...
    assert!(
        main_js().contains("function pluginPresentationForClip(clip)")
            ...
            && styles_css().contains("box-sizing: border-box;")
            && styles_css().contains("border: 1px solid rgba(226, 232, 240, 0.26);")
            && styles_css().contains("border-radius: 7px;")
            && styles_css().contains("background: rgba(15, 23, 42, 0.55);")
