...
33. **League presentation plugin extraction** — the built-in game plugin seam now loads
     ...
     `clipline-plugin-league-of-legends` repository with a v1.3.4 package zip; Clipline pins that
...
2. **Signed plugin metadata:** the external
   `clipline-plugin-league-of-legends` v1.3.4 zip is published and Clipline pins its digest for
